...
public class PageServiceImpl extends TransactionalService implements PageService, ApplicationContextAware {
	...
	@Override
	public List<PageEntity> importFiles(NewPageEntity pageEntity) {
		return importFiles(null, pageEntity);
	}

	@Override
	public List<PageEntity> importFiles(String apiId, NewPageEntity pageEntity) {
...
//			String[] filepath = fetcher.getConfiguration().getFilepath().split("/");
//			if (filepath.length > 0 && graviteeDescriptorService.descriptorName().equals(filepath[filepath.length - 1])) {
//				try {
//					final Resource resource = fetcher.fetch();
...
//			}
	...
	private List<PageEntity> importDescriptor(final String apiId, final NewPageEntity descriptorPageEntity, final FilesFetcher fetcher, final GraviteeDescriptorEntity descriptorEntity) {
	...
	private List<PageEntity> importDirectory(String apiId, NewPageEntity pageEntity, FilesFetcher fetcher) {
		try {
